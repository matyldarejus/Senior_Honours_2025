...
if __name__ == '__main__':
    ...
    fig, ax = plt.subplots(len(lines), 1, figsize=(14, 13), sharey='row', sharex='col')
    ...
    for l, line in enumerate(lines):
        ...
        if os.path.isfile(profile_file):
            ...
            bin_label = 'all'
            mass_mask = np.ones(len(mass_long), dtype=bool)
        ...
        nbins_m = len(mass_bin_labels)

        bin_label = 'all'
        a = ax[l]  # single panel per line

        a.plot(plot_data['fr200'], plot_data[f'{bin_label}_sf_med'], ls='-', c=cb_blue, label='SF', lw=1.5)
        a.fill_between(plot_data['fr200'], plot_data[f'{bin_label}_sf_per75'], plot_data[f'{bin_label}_sf_per25'], alpha=0.3, color=cb_blue)

        a.plot(plot_data['fr200'], plot_data[f'{bin_label}_gv_med'], ls='-', c=cb_green, label='GV', lw=1.5)
        a.fill_between(plot_data['fr200'], plot_data[f'{bin_label}_gv_per75'], plot_data[f'{bin_label}_gv_per25'], alpha=0.3, color=cb_green)

        a.plot(plot_data['fr200'], plot_data[f'{bin_label}_q_med'], ls='-', c=cb_red, label='Q', lw=1.5)
        a.fill_between(plot_data['fr200'], plot_data[f'{bin_label}_q_per75'], plot_data[f'{bin_label}_q_per25'], alpha=0.3, color=cb_red)

        a.set_ylim(-3., 1.)
        a.set_ylabel(r'${\rm log (EW}/\AA)$')
        a.annotate(plot_lines[l], xy=(0.05, 0.85), xycoords='axes fraction')
        if l == 0:
            a.legend(loc=1)
        if l == len(lines) - 1:
            a.set_xlabel(r'$\rho / r_{200}$')

    plt.tight_layout()
    fig.subplots_adjust(wspace=0., hspace=0.)
    plt.savefig(f'{plot_dir}{model}_{wind}_{snap}_ew_profile_no_mass_bins.png')
    plt.show()
    plt.clf()
